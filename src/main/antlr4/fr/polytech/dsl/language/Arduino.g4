...
//Database.add($label.text, "INPUT");
set returns [String texte] :
    label=ID INPUT { $texte = "pinMode(" +$label.text + ", INPUT);";}
    | label=ID OUTPUT { $texte = "pinMode(" +$label.text + ", OUTPUT);";}
;
//Database.add($label.text, "OUTPUT");
